...
logging_path = os.path.join(script_dir, '..', config['logging_path'])
...
logging.debug(f"Logging path: {logging_path}")

# Chemins vers les dossiers principaux
chemin_images = os.path.join(script_dir, '..', '..', config['images_path'])
chemin_transcriptions = os.path.join(script_dir, '..', config['transcriptions_path'])

logging.debug(f"Images path: {chemin_images}")
logging.debug(f"Transcriptions path: {chemin_transcriptions}")

# Chemin du fichier JSON pour stocker les données
json_output_path = os.path.join(script_dir, '..', config['json_output_path'])

logging.debug(f"JSON output path: {json_output_path}")
...
if not os.path.exists(json_output_path):
    with open(json_output_path, 'w') as json_file:
        json.dump([], json_file)
        logging.info(f"Created new JSON file at: {json_output_path}")
...
def save_to_db_and_json(image_data, json_output_path):
    ...
    try:
        db_path = os.path.join(script_dir, config['db_path'])
        ...
        if os.path.exists(json_output_path) and os.path.getsize(json_output_path) > 0:
            with open(json_output_path, 'r') as json_file:
                ...
                logging.debug(f"Loaded existing JSON data: {transcriptions}")
        else:
            transcriptions = []
